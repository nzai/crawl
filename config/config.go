...
func OpenFile(filePath string) ([]*Config, error) {
	...
	if err != nil {
		zap.L().Error("read file failed", zap.Error(err), zap.String("path", filePath))
		return nil, err
	}
...
func (c Config) Strings(key string) ([]string, error) {
	...
	for index, value := range array {
		...
		if !ok {
			return nil, fmt.Errorf("key [%s] value %+v is not a string array", key, v)
		}
...
func (c Config) Config(key string) (*Config, error) {
	...
	if !ok {
		return nil, fmt.Errorf("key [%s] value %+v is not a map[string]interface{}", key, v)
	}
...
func (c Config) Configs(key string) ([]*Config, error) {
	...
	if !ok {
		return nil, fmt.Errorf("key [%s] value %+v is not a interface{} array", key, value)
	}
	...
	for index, item := range array {
		...
		config, ok := item.(map[string]interface{})
		if !ok {
			return nil, fmt.Errorf("key [%s] value %+v is not a map[string]interface{}", key, value)
		}
...
func (c Config) StringParameter(key string, ctx *context.Context) (string, error) {
	...
	default:
		return "", fmt.Errorf("invalid string type: %s", _type)
	}
}
...
func (c Config) IntParameter(key string, ctx *context.Context) (int, error) {
	...
	default:
		return 0, fmt.Errorf("invalid int type: %s", _type)
	}
}
